...
//
// Adding the gRPC generators required one, because the graph genuinely changed.
// Every generator here is on its current release, and the `go` directive below
// is ahead of the published module's on purpose: this module is never published
// and never imported, so its Go requirement reaches nobody.
