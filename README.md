...
**Enrollment (sign-up) is the one trust-on-first-use write.** No key is enrolled yet, so `SignUpRequest` carries the enrolling `public_key` inline (base64url SPKI DER) — on the request, not on every `Signature`. The store binds it under `user_signature.key_id`, which must thumbprint it; the assertion proves possession. For a passkey this is the `navigator.credentials.create()` ceremony's public key.
...
- **No write path verifies a signature yet.** What persistence owes is stated only in field comments and the verify procedure above: resolve `keyId` to the enrolled key, recompute the digest and confirm the challenge binding, verify the assertion, apply the per-layer acceptance policy (participant: `webauthn.get`, a known origin, user-verified; institution: the countersign type, no origin), check at sign-up that `keyId` thumbprints the enrolling `public_key`, refuse a vote whose `user_id` is not the resolved author, bind `authenticatorData`'s rpId hash to the expected RP per environment, and hold a `time`/`recorded` tolerance in chaincode config. `signing.VerifyUser`/`VerifyCountersign` implement the crypto and the acceptance policy; the rpId-hash binding is left to the verifier's config, and no repository wires any of it into a write path yet.
