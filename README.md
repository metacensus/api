...
Each resource file declares its own routes: `topic.proto` has `TopicRoutes`, `paper.proto` has `PaperRoutes`, and so on across all eight. The public package adds `PartnerRoutes` in `partner.proto`. They declare 25 routes between them, in one manifest.
