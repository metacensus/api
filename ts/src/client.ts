...
// exactly once, so `ClientRequest.body` is the string that goes on the wire.
// The caller owns everything about the transport itself: auth headers and
...
//
// Signing is not among them. A participant's signature is a field of the
// request message, made before a client method is called, and is verified
// behind persistence rather than at the edge — see @metacensus/api/signing.
//
...
// The caller supplies this: how a request actually goes over the wire. This is
// where auth headers and the SPA's 401 policy belong — none of that is the
// contract's concern, and a signature is not among them: it travels inside the
// request message. One type for both surfaces, though a real deployment passes
// a different implementation to each: the public surface sends no credentials.
