...
the signer returns the enrolling `publicKey`, which the client sends on the
request (not on the signature — no record carries its own verification key) and
the store binds under the signature's `keyId`. Keep the password out of
`content`, since content is what gets stored.
